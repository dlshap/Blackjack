...
public class BasicStrategy implements Strategy {
	...
	private Play[][] playForPairs = new Play[10][10];
	...
	public Play getPlay(Card dealerCard, Hand playerHand) {
		...
		// if it is a pair, get the next play from the pair strategy array
		if (hand.get(0).rank().equals(hand.get(1).rank()))
			play = playForPairs[playerHand.getHand().get(0).faceValue()-1][dealerCard.faceValue()-1];
		return play;
	}
	
	private void buildPlayForPairs() {
		for (int i=0; i<10; i++) { 
			// Always split aces
